...
class CLEFT():
    ...
    def __init__(self, k = None, p = None, pfile = None, qfile = None, rfile = None, ensfile = None, \
        ...
        self.jn = 20 #number of bessels to sum over #initially 20
...
def make_table(cl, kmin = 1e-3, kmax = 3, nk = 100, npool = 2, z = 0, M = 0.3):
 ...
 P_dd^2    P_s^2    P_ds^2    P_d^2s^2   P_s^2s^2   P_D2d     P_dD2d"
    ...
    pool = mp.Pool(npool)
    #kkk=0.01
    #paa=-np.exp(-0.5*kkk*kkk*(XYlin))*(kkk**2)*(qv**2)*(Xloop + Yloop - 2*1*Yloop/Ylin/(kkk*2.))*(kkk*kkk*Ylin/kkk/qv)
    #datatest=numpy.array([qv, paa])
    #datatest=datatest.T
    #numpy.savetxt('testPA',datatest)
    pktable[:, 0] = kv[:]        
...
def aloop( k, l):
    #print (Yloop.shape)
    #print (-(Xloop + Yloop - 2*l*Yloop/Ylin/k**2.))
