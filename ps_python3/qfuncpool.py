...
class Qfunc:
    ...
    def __init__(self, k, p, Qfile = None, Rfile = None, npool = 4, extrapker = True, saveqfile = None):
        ...
        self.kint = numpy.logspace(-4, 3, 1e4) #numpy.logspace(-4, 3, 1e4)
        self.npool = npool
        self.extrapker = extrapker #If the kernels should be extrapolated. True recommended
        if Qfile is None:
            self.kq, self.Q1, self.Q2, self.Q3, self.Q5, self.Q8, self.Qs2, self.QI, self.Q7, self.Q9, self.Q11, self.Q12, self.Q13 = self.calc_Q()
        else:
        ...
        #Extra SPT functions by George-not necessary for CLPT
        self.ilQI = self.loginterp(self.kq, self.QI, rp = -5)
        self.ilQ7 = self.loginterp(self.kq, self.Q7, rp = -5)
        self.ilQ9 = self.loginterp(self.kq, self.Q9, rp = -5)
        self.ilQ11 = self.loginterp(self.kq, self.Q11, rp = -5)
        self.ilQ12 = self.loginterp(self.kq, self.Q12, rp = -5)
        self.ilQ13 = self.loginterp(self.kq, self.Q13, rp = -5)        
       
        if Rfile is None:
            self.kr, self.R1, self.R2, self.R3, self.R4 = self.calc_R()
        else:
            self.kr, self.R1, self.R2, self.R3, self.R4  = numpy.loadtxt(Rfile, unpack=True)
        self.ilR1 = self.loginterp(self.kr, self.R1)
        self.ilR2 = self.loginterp(self.kr, self.R2)
        self.ilR3 = self.loginterp(self.kr, self.R3)
        self.ilR4 = self.loginterp(self.kr, self.R4)
    ...
    def calc_Q(self):
        ...
        k = numpy.logspace(-3, 2, 5e2)
        ns = [1, 2, 3, 5, 8, -1, 20, 7, 9, 11, 12, 13]
        Qv = ksp.Q(kv = k, pk = self.ilpk, npool = self.npool, ns = ns)
        return Qv[0], Qv[1], Qv[2], Qv[3], Qv[4], Qv[5], Qv[6], Qv[7], Qv[8], Qv[9], Qv[10], Qv[11], Qv[12]
    ...
    def dosph(self, n, x, f, tilt = 1.5, extrap = True, q1 = 1e-3, q2 = 5e2): #extrap was set to True by default, George's comment, q1 = 1e-3, q2 = 5e2
        f = f*self.renorm
        if not self.extrapker:
            ...
            #print (q,p)
            qv = numpy.logspace(-6, 7, 1e4)# (-6, 7, 1e4)
            #qv = numpy.logspace(numpy.log10(1.0028147278e-04), numpy.log10(9.971931737E+05), 4096)# (-6, 7, 1e4)
    ...
    #0 lag
    def xi0lin0(self, kmin = 1e-6, kmax = 1e3):   #kmin = 1e-3, kmax = 1e2
        val = quad(self.ilpk, kmin, kmax, limit = 200)[0]/self.tpi2
        return val

    def xi0loop0(self, kmin = 1e-3, kmax = 1e2):
        val = self.xi0loop013() + self.xi0loop022()
        return val

    def xi0loop013(self, kmin = 1e-3, kmax = 1e2):
        kint = numpy.logspace(numpy.log10(kmin), numpy.log10(kmax), 1e3)
        integrand = 10./21.*self.ilR1(kint)
        linterp = self.loginterp(kint, integrand)
        val = quad(linterp, kmin, kmax, limit = 200)[0]/self.tpi2
        return val

    def xi0loop022(self, kmin = 1e-3, kmax = 1e2):
        kint = numpy.logspace(numpy.log10(kmin), numpy.log10(kmax), 1e3)
   ...
   #j0
    def xi0lin(self, kint = None, tilt = 1.5):
        if kint is None:
            #kint = self.kint
            kint=numpy.logspace(-4, 4, 1e4)
        integrand = self.ilpk(kint)
        integrand /= (kint**2.*self.tpi2)
        #test
        #q, p = sph(kint, 0, q = 1.5)(integrand*self.renorm, extrap = True)
        #q1, q2 = numpy.where(q>1e-1)[0][0], numpy.where(q>100)[0][0] 
        #print (p[q1:q2])
        #qv = numpy.logspace(-6, 7, 1e4) 
        #ip = self.loginterp(q[q1:q2], p[q1:q2])
        #print (numpy.where(ip(qv)<-1e7))
        #print (qv, ip(qv))
        #datatest=numpy.array([q,p])
        #datatest=datatest.T
        #numpy.savetxt('testbug',datatest)
        #endtest
        return self.dosph(0, kint, integrand, tilt = tilt, q1 = 1e-3, q2 = 1000)#2000 initially
    ...
    def xi2lin(self, kint = None, tilt = 1.5):
        ...
        integrand = self.ilpk(kint)
        integrand /= (kint**2.*self.tpi2)
        return self.dosph(2, kint, integrand, tilt = tilt, q1 = 1e-1, q2 =700)#2000 initially
    ...
    def xi2loop13(self, kint = None, tilt = 1.5):
        ...
        integrand /= (kint**2 *self.tpi2)
        #test
        #q, p = sph(kint, 2, q = 1.5)(integrand*self.renorm, extrap = True)
        #q1, q2 = numpy.where(q>1e-3)[0][0], numpy.where(q>500)[0][0] 
        #print (p[q1:q2])
        #qv = numpy.logspace(-6, 7, 1e4) 
        #ip = self.loginterp(q[q1:q2], p[q1:q2])
        #print (numpy.where(ip(qv)<-1e7))
        #print (qv, ip(qv))
        #datatest=numpy.array([qv,ip(qv)])
        #datatest=datatest.T
        #numpy.savetxt('testbug',datatest)
        #endtest
        return self.dosph(2, kint, integrand, tilt = tilt, q1 = 1e-3, q2 =800)#George had it to 700
    ...
    def xi3loop22(self, kint = None, tilt = 1.5):
        ...
        integrand *= (-3./7.)/(kint**3. *self.tpi2)
        #print (self.dosph(3, kint, integrand, tilt = tilt, q1 = 1e-2, q2 =2000))
        return self.dosph(3, kint, integrand, tilt = tilt, q1 = 1e-2, q2 =2000)
    ...
    def u3(self, kint = None, tilt = 1.5):
        ...
        integrand /= (kint**2.*self.tpi2)
        return self.dosph(1, kint, integrand, tilt = tilt, q1 = 1e-5, q2 = 500)#initially q2=600
    ...
    def u11(self, kint = None, tilt = 1.5):
        ...
        integrand = (-6./7)* kint* (self.ilR3(kint))
        integrand /= (kint**2.*self.tpi2)
        #test_George
        q, p = sph(kint, 1, q = 1.5)(integrand*self.renorm, extrap = True)
        q1, q2 = numpy.where(q>1e-5)[0][0], numpy.where(q>200)[0][0] 
        #print (p[q1:q2])
        #qv = numpy.logspace(-6, 7, 1e4) 
        #ip = self.loginterp(q[q1:q2], p[q1:q2])
        #print (numpy.where(ip(qv)<-1e7))
        ...
        #numpy.savetxt('testbug',datatest)
        #endtest
        return self.dosph(1, kint, integrand, tilt = tilt, q1 = 1e-5, q2 = 200)

    def u20(self, kint = None, tilt = 1.5): # q, p = sph(x, nu = n, q = tilt)(f, extrap = extrap)
        if kint is None:
            kint = self.kint
        kint=numpy.logspace(-4, 4, 1e4)
        ...
        #test_George
        #q, p = sph(kint, 1, q = 1.5)(integrand*self.renorm, extrap = True)
        #q1, q2 = numpy.where(q>1e-5)[0][0], numpy.where(q>900)[0][0] 
        #print (p[q1:q2])
        #qv = numpy.logspace(-6, 7, 1e4) 
        #ip = self.loginterp(q[q1:q2], p[q1:q2])
        #print (numpy.where(ip(qv)<-1e7))
        ...
        #numpy.savetxt('testbug',datatest)
        #endtest
        return self.dosph(1, kint, integrand, tilt = tilt, q1 = 1e-5, q2 = 700) #q2=1000 reproduces error!
    ...
    def x100lag(self, kmin = 1e-3, kmax = 1e2, kint = None):
        if kint is None:
            kint = self.kint
        #integrand = 1/7.*(self.ilR1(kint) - self.ilR2(kint))
        integrand = 1/7.*(self.ilR4(kint) - self.ilR2(kint))
        linterp = self.loginterp(kint, integrand)
        val = quad(linterp, kmin, kmax, limit = 200)[0]/self.tpi2
        return val
    ...
    def x10j0(self, kint = None, tilt = 1.5):
        ...
        integrand = (-1./14)*(2*self.ilR2(kint) + 2*self.ilR3(kint)+ 2*self.ilQ5(kint))
        integrand /= (kint**2.*self.tpi2)
        return self.dosph(0, kint, integrand, tilt = tilt, q1 = 1e-5, q2 =500)#initially q2=500

    def x10j2(self, kint = None, tilt = 1.5):
        if kint is None:
            kint = self.kint
        #integrand = (-1./14)*(3*self.ilR1(kint) + 4*self.ilR2(kint) + 2*self.ilQ5(kint))
        integrand = (-1./14)*(self.ilR4(kint) + 2*self.ilR2(kint) + 2*self.ilR3(kint)+ 2*self.ilQ5(kint))
        integrand /= (kint**2.*self.tpi2)
        return self.dosph(2, kint, integrand, tilt = tilt, q1 = 1e-3, q2 =500)
    ...
    def y10(self, kint = None, tilt = 1.5):    
        ...
        integrand = (3./14)*(self.ilR4(kint) + 2*self.ilR2(kint)+ 2*self.ilR3(kint) + 2*self.ilQ5(kint))
        integrand /= (kint**2.*self.tpi2)
        #Addhoc factor of 2 that I seem to be missing
        integrand *= 2.
        return self.dosph(2, kint, integrand, tilt = tilt, q1 = 1e-3, q2 =500)
    ...
    def t112(self, kint = None, tilt = 1.5):
        ...
        qt = self.ilQI(kint) + 2*self.ilQ2(kint)
        rt = 2.*self.ilR4(kint) + 4*self.ilR2(kint)
        integrand = qt + rt 
        integrand *= (-3./7.)/(kint**3. *self.tpi2)
    ...
    def save_qfunc(self, fname):
        ...
        #         '{0: ^13}'.format('E0loop0') + '{0: ^13}'.format('E0loop')  + \
        #         '{0: ^13}'.format('E2loop') + '{0: ^13}'.format('E1loop') + '{0: ^13}'.format('E3loop') + \
        ...
        #         '{0: ^13}'.format('X10') + '{0: ^13}'.format('Y10') + '{0: ^13}'.format('X20') + '{0: ^13}'.format('Y20')  + '{0: ^13}'.format('Y20') + \
        #         '{0: ^13}'.format('Y20') + '{0: ^13}'.format('Y20') + '{0: ^13}'.format('Y20') + '{0: ^13}'.format('Y20')       
        header = '{0: ^13}'.format('q') +  '{0: ^13}'.format('E0lin') + '{0: ^13}'.format('Elin') + '{0: ^13}'.format('E2lin') + \
        ...
        tosave = numpy.array([qv, xi0lin0 + qv*0, xi0lin, xi2lin, xi0loop022 + xi0loop013 + qv*0, xi0loop22 + xi0loop13, 
        ...
        #                      xi2loop22, xi2loop13, xi1loop, xi3loop, corr, zeta, chi, u10, u3, u11, u20, v10, v12, x10, y10, x20, y20, y20, y20]).T
        numpy.savetxt(fname, tosave, header = header, fmt = '%0.6e')
        #numpy.savetxt(fname, tosave, fmt = '%0.6e')
